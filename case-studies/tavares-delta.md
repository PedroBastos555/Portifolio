...
- **Decisão:** RLS por `company_id` em vez de banco por tenant
  **Por quê:** Foi escolhida pensando em viabilidade — manter um banco de dados separado por cliente traria custo operacional desnecessário nesta fase do projeto. Como sou o único dev, RLS por `company_id` também reduz a complexidade de manutenção: um único schema para migrar, monitorar e evoluir.
